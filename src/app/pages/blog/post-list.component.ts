...
@Component({
  ...
  template: `
    ...
    <p class="lede">Entradas desde <code>assets/content/posts.json</code>.</p>

    @if (posts$ | async; as posts) {
      @if (posts.length === 0) {
        <p class="empty-msg">No hay posts publicados.</p>
      } @else {
        <div class="list">
            ...
            <mat-card class="surface-card row" appearance="outlined">
              ...
              @if (post.tags?.length || post.draft) {
                <mat-card-content>
                  <mat-chip-set>
                    @if (post.draft) {
                      <mat-chip highlighted color="warn">Borrador</mat-chip>
                    }
